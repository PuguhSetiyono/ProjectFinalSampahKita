      ...
      <div class="flex-grow">
        ...
        <div class="p-4">
          <ul class="space-y-1">
            ...
            <li>
             <a href="{{ route('admin.paket') }}" class="flex items-center bg-green-200 rounded-xl font-bold text-sm text-green-900 py-3 px-4">
                <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" class="text-lg mr-4" viewBox="0 0 16 16">
    ...
    <div class="px-6 py-8">
        <div class="max-w-7xl mx-auto">
            <div class="bg-white rounded-3xl px-8 py-5 mb-5">
                <h1 class="text-3xl font-bold mb-3">Edit Paket</h1>
                <hr class="mt-1">
                    ...
                    <div class="w-full py-5 bg-white">
                        <form action="{{ route('admin.paket.update', $paket->id) }}" method="POST">
                            ...
                            <div>
                                ...
                                <input type="text" id="nama_paket" name="nama_paket" value="{{ $paket->nama_paket }}" class="border rounded-md p-2 w-full" required>
                            </div>
                            <div>
                                <label for="harga_paket">Harga Paket:</label>
                                <input type="text" id="harga_paket" name="harga_paket" value="{{ $paket->harga_paket }}" class="border rounded-md p-2 w-full" required>
                            </div>
                            <div>
                                <label for="armada">Armada:</label>
                                <input type="text" id="armada" name="armada" value="{{ $paket->armada }}" class="border rounded-md p-2 w-full" required>
                            </div>
                            <div>
                                <label for="kapasitas_armada">Kapasitas Armada:</label>
                                <input type="text" id="kapasitas_armada" name="kapasitas_armada" value="{{ $paket->kapasitas_armada }}" class="border rounded-md p-2 w-full" required>
                            </div>
                            <div>
                                <label for="id_jadwal">ID Jadwal:</label>
                                <select id="id_jadwal" name="id_jadwal" class="border rounded-md p-2 w-full" required>
                                    @foreach($jadwals as $jadwal)
                                        <option value="{{ $jadwal->id }}" @if($jadwal->id === $paket->id_jadwal) selected @endif>
                                            {{ $jadwal->hari }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="text" id="gambar" name="gambar" value="{{ $paket->gambar }}" class="border rounded-md p-2 w-full" required>
                            </div>
                            <div>
                                <label for="durasi_paket">Durasi Paket:</label>
                                <input type="text" id="durasi_paket" name="durasi_paket" value="{{ $paket->durasi_paket }}" class="border rounded-md p-2 w-full" required>
                            </div>
                            <!-- ... -->
                            <div class="my-4">
