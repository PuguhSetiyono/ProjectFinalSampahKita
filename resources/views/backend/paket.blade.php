    ...
    <div class="px-6 py-8">
        <div class="max-w-7xl mx-auto">
            <div class="bg-white rounded-3xl px-8 py-5 mb-5">
                <h1 class="text-3xl font-bold mb-3">Data Paket</h1>
                <hr class="mt-1">
                    ...
                    <table class="min-w-full mt-4 bg-white border border-gray-300">
                        <thead>
                            <tr>
                                ...
                                <th class="py-2 px-4 border-b">Armada</th>
                                <th class="py-2 px-4 border-b">Kapasitas Armada</th>
                                <!-- <th class="py-2 px-4 border-b">ID Jadwal</th> -->
                                <th class="py-2 px-4 border-b">Gambar</th>
                                <th class="py-2 px-4 border-b">Nama Paket</th>
                                <th class="py-2 px-4 border-b">Harga Paket</th>
                                <th class="py-2 px-4 border-b">Durasi Paket</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="py-2 px-4 border-b">{{ $paket->id }}</td>
                                <td class="py-2 px-4 border-b">{{ $paket->armada }}</td>
                                <td class="py-2 px-4 border-b">{{ $paket->kapasitas_armada }}</td>
                                <!-- <td class="py-2 px-4 border-b">{{ $paket->id_jadwal }}</td> -->
                                <td class="py-2 px-4 border-b">{{ $paket->gambar }}</td>
                                <td class="py-2 px-4 border-b">{{ $paket->nama_paket }}</td>
                                <td class="py-2 px-4 border-b">{{ $paket->harga_paket }}</td>
                                <td class="py-2 px-4 border-b">{{ $paket->durasi_paket }}</td>
                                <td class="py-2 px-4 border-b flex space-x-2">
                                    <a href="{{ route('admin.paket.edit', $paket->id) }}" class="bg-yellow-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">Edit</a>
                                    <form action="{{ route('admin.paket.delete', $paket->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
